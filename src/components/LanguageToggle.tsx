...
const LanguageToggle = () => {
  ...
  return (
    ...
    >
      <span className={`transition-all duration-200 ${lang === "he" ? "font-semibold text-accent-purple" : "text-muted-foreground"}`}>
        עברית
      </span>
      <span className="text-border mx-1">|</span>
      <span className={`transition-all duration-200 ${lang === "en" ? "font-semibold text-accent-purple" : "text-muted-foreground"}`}>
        EN
      </span>
    </button>
  );
